...
- `a8apps/*` — Atari **guest** software (games, demos; eventually OneDOS): programs that run _inside_ the emulated machine, as opposed to `apps/*` which run on the host. Private workspace members like apps (`workspace:*` deps, bespoke toolchains), but the toolchain is spasm plus small Node build scripts, the artifact is an Atari image (`.xex`, ATR, cartridge), and tests exercise the built image (structurally, and eventually by booting it in the a8 core). Never published; no publint/tsdown.

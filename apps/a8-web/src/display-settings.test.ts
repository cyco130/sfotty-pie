...
test("the full picture crops to the standard 240 lines", () => {
	// Rows are vcount: the Atari's drawable 240 lines are rows 8..247.
	expect(overscanCrop({ width: 376, height: 240 }, "ntsc")).toEqual({
		left: 0,
		top: 8,
		width: 376,
		height: 240,
...
test("a full-scan crop covers the whole buffer", () => {
	// Taller than the display centre allows: the crop slides down to row 0.
	expect(overscanCrop({ width: 376, height: 262 }, "ntsc")).toEqual({
		left: 0,
		top: 0,
		width: 376,
		height: 262,
	});
	expect(overscanCrop({ width: 376, height: 312 }, "pal")).toEqual({
		left: 0,
		top: 0,
		width: 376,
		height: 312,
...
test("no-overscan is exactly the OS playfield window", () => {
	...
	// 32..223 -> buffer rows 32..223.
	expect(overscanCrop({ width: 320, height: 192 }, "ntsc")).toEqual({
		left: 28,
		top: 32,
		width: 320,
		height: 192,
...
test("336x208 sits centred on the playfield, not the buffer", () => {
	// 336 -> absolute 88..423 -> buffer left 20; 208 -> vcount 24..231.
	expect(overscanCrop({ width: 336, height: 208 }, "pal")).toEqual({
		left: 20,
		top: 24,
		width: 336,
		height: 208,
...
test("sanitize clamps to the legal range and even values", () => {
	expect(sanitizeOverscan({ width: 300, height: 300 }, "ntsc")).toEqual({
		width: 320,
		height: 262,
	});
	expect(sanitizeOverscan({ width: 300, height: 300 }, "pal")).toEqual({
		width: 320,
		height: 300,
	});
	expect(sanitizeOverscan({ width: 400, height: 100 }, "ntsc")).toEqual({
		width: 376,
		height: 192,
	});
	expect(sanitizeOverscan({ width: 335, height: 209 }, "ntsc")).toEqual({
		width: 334,
		height: 208,
	});
});

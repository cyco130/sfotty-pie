...
[Launch the emulator](/a8/emu). The first time you click or tap the screen, Sfotty Pie turns the sound on - browsers only allow audio after you interact with the page, so if it's silent at first, give it a click.
...
**Typing.** By default the keyboard is in **Character mode**: you type normally and your letters land as the matching Atari keys. Some games instead expect certain _physical_ keys (like the keys where W, A, S, D sit), so switch to **Positional mode** for those, from **Menu → Keyboard shortcuts…**.

**Pasting text.** Paste your computer's clipboard straight into the Atari with **Cmd+V** on a Mac or **Alt+V** on Windows and Linux (Ctrl+V belongs to the Atari - it types the real Ctrl+V key). The text is typed through the Atari's own operating system, so it lands anywhere the machine reads the keyboard: paste a BASIC listing at the READY prompt and watch it type itself in. Firefox and Safari ask for a click on a small **Paste** confirmation first. A few tricks for listings: `~` toggles inverse video, `{125}` or `{$7D}` inserts any ATASCII code by number (shown as its glyph rather than performed - write `{!125}` to make a control code act), and Atari glyphs like `♥` in the pasted text come through as themselves.
...
Sfotty Pie runs the same in a mobile browser - nothing to install. On a touch screen you get **on-screen controls**: a joystick and fire button, the console keys (Start, Select, Option, Reset), a keyboard, and a **paste** tab - type or paste text there (long-press for your phone's own paste) and send it to the Atari. You can show or hide the controls, and swap the stick and fire to whichever side suits you, from the on-screen menu.

Collecting software is easier on a computer, so a handy trick is to build and organize your **Library** there, **Export** it to a single `.zip`, then **Import** that `.zip` on your phone - the whole collection comes across at once.

One thing to know: your library and settings live in your browser's own storage, and browsers sometimes clear that storage to reclaim space. Keep an exported `.zip` as a backup so nothing is ever truly lost.

...
test("the XL self-test window requires the OS ROM to be enabled", () => {
	...
	machine.mmu.write(0xd301, 0x7f, ReadOptions.NONE);
	expect(machine.mmu.read(0x5000, ReadOptions.NONE)).toBe(0xaa);

	// With the OS ROM banked out, the self-test vanishes too - it lives on
	// the OS ROM chip. RAM shows through instead.
	machine.mmu.write(0xd301, 0x7e, ReadOptions.NONE);
	expect(machine.mmu.read(0x5000, ReadOptions.NONE)).toBe(0x00);
...
test("the 130XE separates CPU and ANTIC extended RAM access", () => {
	...
	// main RAM (bit 5 high).
	machine.mmu.write(0xd301, 0xe3, ReadOptions.NONE);
	machine.mmu.write(0x4000, 0x55, ReadOptions.NONE); // lands in extended bank 0
	expect(machine.mmu.read(0x4000, ReadOptions.NONE)).toBe(0x55);
	expect(machine.mmu.read(0x4000, ReadOptions.DMA)).toBe(0x00); // ANTIC: main

	// Banks are distinct: bank 1 (bit 2 high) is its own RAM.
	machine.mmu.write(0xd301, 0xe7, ReadOptions.NONE);
	expect(machine.mmu.read(0x4000, ReadOptions.NONE)).toBe(0x00);
	machine.mmu.write(0xd301, 0xe3, ReadOptions.NONE);
	expect(machine.mmu.read(0x4000, ReadOptions.NONE)).toBe(0x55);
	...
	machine.mmu.write(0xd301, 0xd3, ReadOptions.NONE);
	expect(machine.mmu.read(0x4000, ReadOptions.DMA)).toBe(0x55);
	expect(machine.mmu.read(0x4000, ReadOptions.NONE)).toBe(0x00);
});

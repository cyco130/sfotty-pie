...
test("joystick 0/1 directions drive PORTA, active low", () => {
	...
	machine.mmu.write(PACTL, 0x04, ReadOptions.NONE);
	expect(machine.mmu.read(PORTA, ReadOptions.NONE)).toBe(0xff);
	...
	expect(machine.mmu.read(PORTA, ReadOptions.NONE)).toBe(0x7a);

	stick0.release(0x01); // release up, left stays held
	expect(machine.mmu.read(PORTA, ReadOptions.NONE)).toBe(0x7b);
...
test("assigning direction replaces the stick position atomically", () => {
	...
	expect(machine.mmu.read(PORTA, ReadOptions.NONE)).toBe(0xf6);
	expect(stick0.direction).toBe(0x09);

	stick0.direction = 0x02; // down only - up+right implicitly released
	expect(machine.mmu.read(PORTA, ReadOptions.NONE)).toBe(0xfd);
...
test("the 800 has joysticks 2/3 on PORTB", () => {
	...
	expect(machine.mmu.read(PORTB, ReadOptions.NONE)).toBe(0xed);
});

test("triggers drive the GTIA TRIG lines", () => {
	const machine = makeMachine("800");
	expect(machine.mmu.read(TRIG0, ReadOptions.NONE)).toBe(1);
	...
	stick0.trigger = true;
	stick2.trigger = true;
	expect(machine.mmu.read(TRIG0, ReadOptions.NONE)).toBe(0);
	expect(machine.mmu.read(TRIG2, ReadOptions.NONE)).toBe(0);
...
test("the XL has no ports 2/3", () => {
	...
	expect(machine.mmu.read(PORTB, ReadOptions.NONE)).toBe(0xff);
	expect(machine.mmu.read(TRIG3, ReadOptions.NONE)).toBe(0);
});

test("XL/XE TRIG3 senses the cartridge (RD5)", () => {
	expect(makeMachine("800XL").mmu.read(TRIG3, ReadOptions.NONE)).toBe(0);
	...
	expect(withCart.mmu.read(TRIG3, ReadOptions.NONE)).toBe(1);
});

test("a stick switch pulls even an output-driven PORTA pin low", () => {
	const machine = makeMachine("800");
	machine.mmu.write(PORTA, 0xff, ReadOptions.NONE); // PACTL bit 2 is 0 after power-on: sets DDRA
	machine.mmu.write(PACTL, 0x04, ReadOptions.NONE);
	machine.mmu.write(PORTA, 0xff, ReadOptions.NONE); // the output latch

	plugStick(machine, 0).press(0x01);
	expect(machine.mmu.read(PORTA, ReadOptions.NONE)).toBe(0xfe);
});
...
test("joystick state survives a reset (switches are physical)", () => {
	...
	machine.mmu.write(PACTL, 0x04, ReadOptions.NONE);
	expect(machine.mmu.read(PORTA, ReadOptions.NONE)).toBe(0xf7);
});

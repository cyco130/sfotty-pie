...
export interface SfottyOptions {
	...
	/**
	 * Specifies whether the CPU should reject undocumented opcodes.
	 *
	 * Undocumented opcodes are not part of the official 6502 specification and
	 * their behavior may vary between different implementations. They execute
	 * by default; set this option to `true` to make every undocumented opcode
	 * behave like `CIM` (also called `KIL`, `JAM`, or `HLT`) and jam the CPU.
	 *
	 * @default false
	 */
...
export class SfottyCore {
	 ...
	 * warm reset leaves them as-is. Also clears a CIM crash. A new CPU already
	 * starts in this sequence (construction is a power-on, i.e. a cold reset);
	 * call this to model a RES pulse at runtime.
	 */
...
/**
 * The default microcode table, with every undocumented opcode's 8-slot block
 * replaced by a copy of CIM's, so each runs the genuine five-cycle jam
 * sequence, bus accesses included. The steps are position-independent (they
 * only ever advance `state` by one or repeat it), so the copy works in any
 * block. `decode` still sets `state = opcode << 3`, so the jammed opcode is
 * preserved in `state` (and reported by `describeState`).
 */
const MICROCODE_CRASH_UNDOCUMENTED: Step[] = MICROCODE.slice();
const CIM_BLOCK = 0x02 << 3;
for (let opcode = 0; opcode < 0x100; opcode++) {
	if (NMOS_INSTRUCTIONS[opcode]!.undocumented) {
		for (let cycle = 0; cycle < 8; cycle++) {
			MICROCODE_CRASH_UNDOCUMENTED[(opcode << 3) | cycle] =
				MICROCODE[CIM_BLOCK | cycle]!;
		}
	}
}

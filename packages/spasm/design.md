...
**Expression macros** are function-valued symbols: `DOUBLE(v) = 2 * v` defines one (the parser speculates on `(` after a statement-position identifier and backs out to the instruction parse when the shape isn't `params) =`), and `DOUBLE(2)` applies it (an identifier or `::` path hugging `(` is a call). The `FunctionValue` joins the `Value` union, interned per definition site so the fixpoint's identity comparison converges; the only operations are application and assignment aliasing (`D = DOUBLE`). Application is eager (args evaluate in the caller's scope), hygienic (body free names resolve in the defining module, by defaulting the origin), pure, and depth-capped against runaway recursion. Function params shadow code-macro substitution inside bodies. Functions are rejected as data, operands, and attribute values, and are filtered out of the public `symbols` map.

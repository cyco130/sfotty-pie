...
- **Addresses as a value type** - `Value` is `bigint | string | FunctionValue`, and label-vs-constant lives on the _binding_ (`SymbolKind`) rather than the value, so `C = L` launders an address into a plain number. Making an address a value (carrying its relocation base and its attributes) is what would let attributes survive arithmetic, give relocation the address/constant distinction it needs at each emit site, and eventually retire `SymbolKind` entirely.

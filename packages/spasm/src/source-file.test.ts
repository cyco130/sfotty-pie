...
describe("formatMessage", () => {
	test("without showLine, just the file:line:col header", () => {
		...
		expect(sf.formatMessage(4, 7, "error", "oops")).toBe(
			"f.s:2:1 - error: oops",
		);
	});
	...
	test("with showLine, a gutter-prefixed excerpt and a squiggle marker", () => {
		const sf = new SourceFile("f.s", "abc\ndef");
		expect(sf.formatMessage(4, 5, "error", "bad", { showLine: true })).toBe(
			"f.s:2:1 - error: bad\n\n2 def\n  ~",
		);
	});
	...
	test("one squiggle per byte of the span", () => {
		...
		expect(sf.formatMessage(0, 3, "error", "x", { showLine: true })).toBe(
			"f.s:1:1 - error: x\n\n1 abcdef\n  ~~~",
		);
	});
	...
	test("tabs in the indent are reproduced in the marker line", () => {
		...
		expect(sf.formatMessage(1, 4, "error", "y", { showLine: true })).toBe(
			"f.s:1:2 - error: y\n\n1 \tlda\n  \t~~~",
		);
	});
	...
	test("a newline token points at the end of the line it terminates", () => {
		...
		expect(lf.formatMessage(2, 3, "error", "nl", { showLine: true })).toBe(
			"f.s:1:3 - error: nl\n\n1 ab\n    ~",
		);
		...
		expect(crlf.formatMessage(2, 4, "error", "nl", { showLine: true })).toBe(
			"f.s:1:3 - error: nl\n\n1 ab\n    ~",
		);
	});
	...
	test("shortName defaults to id but overrides it when given", () => {
		expect(
			new SourceFile("long/path/f.s", "x").formatMessage(0, 1, "error", "m"),
		).toBe("long/path/f.s:1:1 - error: m");
		expect(
			new SourceFile("id", "x", "f.s").formatMessage(0, 1, "error", "m"),
		).toBe("f.s:1:1 - error: m");
	});

	test("color mode paints file, position, kind, gutter, and squiggles", () => {
		const sf = new SourceFile("f.s", "abcdef");
		const out = sf.formatMessage(0, 3, "error", "x", {
			showLine: true,
			color: true,
		});
		expect(out).toContain("\x1b[36mf.s\x1b[0m"); // file: cyan
		expect(out).toContain("\x1b[33m1:1\x1b[0m"); // line:col: yellow
		expect(out).toContain("\x1b[31merror\x1b[0m: x"); // kind: red
	...
	test("the note kind paints gray", () => {
		const sf = new SourceFile("f.s", "abc");
		const out = sf.formatMessage(0, 1, "note", "n", { color: true });
